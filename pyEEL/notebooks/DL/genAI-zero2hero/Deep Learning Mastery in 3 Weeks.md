...
- Build a single neuron, then a full network, with no libraries
- Implement backprop using only NumPy
- Train on a real dataset and visualize decision boundaries
...
- Rebuild your Week 1 network in PyTorch (feel the difference)
- Experiment with He/Xavier init, BatchNorm, Dropout
- Visualize loss landscapes and compare flat vs sharp minima
- Deliberately overfit, then fix it
	**"You know you've got it when..."**
...
- Build a Variational Autoencoder from scratch (pure Python), then in PyTorch
- Generate new images from your trained VAE
- Wrap your model in a Gradio app and deploy it
	**"You know you've got it when..."**
...
- **GitHub repo** with clean notebooks, a trained model, and a README that explains your approach
- **Deployed app** (Gradio on Hugging Face Spaces — it's free)
- **Blog post** walking through your "aha moments" (even a short one on Medium or dev.to)
...
- Code a single neuron with sigmoid activation (5 min)
- Replace sigmoid with ReLU and observe the difference (5 min)
- Train it on a simple AND/OR gate with gradient descent (10 min)
- Answer the challenge question: "Why can't a single neuron learn XOR?" (10 min)
