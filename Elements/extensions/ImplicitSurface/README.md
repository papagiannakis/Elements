...
*author: [Kostis Lymperakis](https://kostis1101.github.io)*
...
The class `MarchingCubes` is a wrapper around the `VertexArray` component. It holds all the necessary functionality to create a mesh (and update the corrisponding `VertexArray`) by approximating the roots of an expression of 3 real variables. It uses a simple implementation of the marching cubes algorithm. Under the hood, the algorithm uses the [`taichi`](https://pypi.org/project/taichi/) module to speed up the generation of the surfaces. If the module is not found, there is a significantly slower native python implementation. Installing the `taichi` module (`pip install taichi`) is strongly recomended for the application to be responsive.
Furthermore, if it exists, it uses the `numexpr` module to speed up the evaluation of the expression. See section "Supported Functions" for more information.
