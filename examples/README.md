...
These examples are suitable for introduction to Computer Graphics and Elements.
  ...
  * [Example B0](./B.Introductory/example_B0_component_system.py): A plain example of a component and a system, without any rendering
  * [Example B1](./B.Introductory/example_B1_coordinate_spaces.py): Points, lines and a triangle on a ground grid, with a panel that prints the selected object's own vertices in every space they pass through on the way to the screen: object → (model) → world → (view) → eye → (projMat) → clip → (divide by w) → NDC → (viewport) → window pixels. The matrices in the panel are the same ones the shaders are handed, so flying the camera leaves the world row untouched and changes everything below it — which is the difference between a transform that belongs to the object and one that belongs to the camera.
  ...
  * [Example B6a](./B.Introductory/example_B6a_lights_pyramid.py): Two square-based pyramids, sharp and smooth, under one point light. Every vertex, index and normal is written out by hand — 18 unshared vertices with a face normal each on the left, 5 shared vertices with averaged normals on the right — which is what `Elements.utils.normals` does for you in B6b.
  * [Example B6b](./B.Introductory/example_B6b_lights_cube.py): The same cube twice, lit by one point light with the Blinn-Phong shader, differing only in its normals: one per face on the left (sharp edges) against one averaged per corner on the right, which reads as rounded despite identical geometry. Both are a single colour, so the shading is all there is to see.
  * [Example B6c](./B.Introductory/example_B6c_lights_sphere.py): The same sphere twice, sharp and smooth, with no geometry written out at all — `geometry_factory` builds the vertices, the indices and both sets of normals. The opposite lesson to B6b: a cube's faces really are flat, so smoothing one is wrong, while a sphere's facets only exist because a curved surface had to be chopped into triangles.
