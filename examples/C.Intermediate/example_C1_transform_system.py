...
Each column isolates one shading model, each row isolates how the normals were generated.
Everything else -- geometry, light, material, camera -- is shared, which is what makes the
comparison mean anything.

  * Phong vs Blinn-Phong: both light every *fragment* and differ in one line of the fragment
    shader. At an equal exponent Blinn-Phong's highlight is the wider of the two; see
    BlinnPhong.frag for why.

  * Gouraud: lights every *vertex* and interpolates the result. Far cheaper -- and on the smooth
    row the highlight all but vanishes, because it is smaller than a triangle and so falls
    between the points where the lighting was sampled. Orbit and it flickers in and out. It only
    does this on a coarse mesh -- see the note on MODEL below.

  * flat vs smooth normals: one normal per triangle reads as faceted, one normal per vertex
    (averaged) reads as curved. Same triangles either way.
...
# A more tolerant OBJ reader than Elements.utils.obj_to_mesh: it accepts v, v/vt and v/vt/vn faces
# and fan-triangulates n-gons, so most .obj files found in the wild just load.
from Elements.extensions.showcase.showcase_helpers import load_obj_mesh

# generateFlatNormalsMesh looks at the *index* array to decide whether to split the mesh, so the
# "flat" row is really flat even for models carrying duplicate vertex positions.
import Elements.utils.normals as norm

# A sphere at a chosen triangle count, rather than loaded from a file -- the default model below.
...
#: Which model fills all six slots. Swap in any .obj with MODEL = obj(MODEL_DIR / "teapot.obj")
#: (cow.obj, teddy.obj and sphere.obj are also bundled); TARGET_SIZE below rescales whatever you
#: pick.
#:
#: The default is a deliberately coarse 252-triangle sphere. Gouraud only breaks visibly when a
#: highlight is about the size of a triangle, and every bundled .obj is far too fine for that --
#: teapot.obj's 6320 triangles render pixel-identically under Gouraud and Phong. Load the teapot
#: to compare Phong against Blinn-Phong, keep the sphere to see Gouraud break.
MODEL = sphere(10, 14)
...
Lintensity = 0.9
# How *strong* the highlight is -- up from the 0.4 the other examples use, since the highlights
# are the whole point here.
Mshininess = 0.9
#: How *tight* it is. All three shaders get the same value, so the shading model is the only thing
#: that varies. Worth raising: the tighter the highlight, the more dramatically Gouraud loses it
#: between vertices. Blinn-Phong needs roughly 4x Phong's exponent to match its width, which is
#: why the middle column looks wider at an equal value.
MspecularExponent = 32.0

# The uniform viewPos is absent here: the specular term depends on where the viewer is, so it is
# read back from the camera every frame in the render loop.
...
# Built once and shared across the three columns, which differ only in the shader they go to.
...
#: (entity name, BasicTransform, ShaderGLDecorator), so the render loop can push the same uniforms
#: to every object without caring which shader it is.
objects = []
...
# of the leftmost column, and the six objects differ only in their shader anyway.
...
example_description = (
    ...
    "Edit MODEL near the top of the file to try another model.\n"
...
while running:
    ...
    # Every object gets the same uniforms -- the three shaders declare the same set, so the shader
    # code is the only thing that varies across the grid.
