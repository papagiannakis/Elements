...
for i, e in enumerate(EdgeVertexIndices):
	EdgeVertexIndices[i] = (min(e), max(e))


import numpy as np
def corner_index_to_coords(c):
	return np.array([c & 1, (c >> 1) & 1, (c >> 2) & 1])


Ttable = [None] * 256

for i, tris in enumerate(triangle_table[1:-1]):
	t = []
	for t0 in tris:
		t.append(
			(
				corner_index_to_coords(EdgeVertexIndices[t0][0]), 
				(EdgeVertexIndices[t0][1] - EdgeVertexIndices[t0][0]) >> 1
			)
		)

	Ttable[i + 1] = t

Ttable2 = np.zeros((256, 4 * 12), dtype=int)

for i, tris in enumerate(Ttable):
	if tris is None:
		continue
	for j, tri in enumerate(tris):
		Ttable2[i, 4 * j + 0] = tri[0][0]
		Ttable2[i, 4 * j + 1] = tri[0][1]
		Ttable2[i, 4 * j + 2] = tri[0][2]
		Ttable2[i, 4 * j + 3] = tri[1]